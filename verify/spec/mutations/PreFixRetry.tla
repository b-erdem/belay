---------------------------- MODULE PreFixRetry ----------------------------
(*****************************************************************************)
(* A TLA+ model of the Belay job engine's DURABLE execution mechanics,    *)
(* drawn from the reference storage implementation                          *)
(* (lib/belay/storage/memory.ex + lib/belay/storage.ex Logic) and the   *)
(* runner (lib/belay/runner.ex), grounded in verify/traces.exs.           *)
(*                                                                          *)
(* The point of the model is the four durable safety mechanisms Belay     *)
...
\* Job 5: a SECOND-LEVEL dependent (deps {3}) so cascade re-dooming and
\*        operator retry are explored across a 2-deep chain.
Program == [ j \in Jobs |->
...
Deps        == [ j \in Jobs |-> CASE j = 3 -> {2} [] j = 5 -> {3} [] OTHER -> {} ]
WfIgnore    == [ j \in Jobs |-> {} ]

StepNames == { "b1", "b2", "b3", "b4", "t1", "s1", "s2", "u1" }
...
VARIABLES
  ...
  retries      \* [Jobs -> 0..1]      operator retries used (bounded for finiteness)

vars == << state, attempt, journaled, spent, cancelReq, live, execCount, clock, retries >>
...
Insert(j) ==
  ...
  /\ UNCHANGED << attempt, journaled, spent, cancelReq, live, execCount, clock, retries >>
...
ClaimJob(j) ==
  ...
  /\ UNCHANGED << journaled, spent, cancelReq, execCount, clock, retries >>
...
Exec(j) ==
  ...
  /\ UNCHANGED << state, attempt, cancelReq, live, clock, retries >>
...
Crash(j) ==
  ...
  /\ UNCHANGED << state, attempt, journaled, spent, cancelReq, execCount, clock, retries >>

\* advance: cosmetic clock tick (leases/backoff are abstracted).
Advance ==
  /\ clock < MaxClock
  /\ clock' = clock + 1
  /\ UNCHANGED << state, attempt, journaled, spent, cancelReq, live, execCount, retries >>
...
CancelRunning(j) ==
  ...
  /\ UNCHANGED << state, attempt, journaled, spent, live, execCount, clock, retries >>
...
\* retry (storage.retry, operator command): failed/cancelled -> ready, or
\* -> held for workflow members (settlement then releases or re-dooms them —
\* skipping that gate was real bug #4). Clears any pending cancel_requested:
\* the retry is the operator's LATER intent (real bug #3 — without this a
\* cooperatively-cancelled job is un-retryable forever). Composes with
\* CancelRequestSurvivesUntilTerminal because that property constrains only
\* NON-terminal states; retry clears the flag FROM a terminal state.
Retry(j) ==
...
\* Property 4.  Terminal states are exited ONLY through the operator retry
\* gate: failed/cancelled -> ready (dep-free) or held (workflow member), and
\* the retry clears any pending cancel — no other terminal exit exists.
TerminalStatesAreFinal ==
  [][ \A j \in Jobs :
        (Terminal(state[j]) /\ state'[j] # state[j]) =>
           /\ state[j] \in { "failed", "cancelled" }
           /\ state'[j] = (IF Deps[j] # {} THEN "held" ELSE "ready")
           /\ ~cancelReq'[j] ]_vars

\* Property 5 (INVARIANT #5, the bug-#4 catcher).  A running job's
\* dependencies are satisfied: released-then-running jobs can never coexist
\* with an unignored failed/cancelled dependency, retry or no retry.
DepGate ==
  \A j \in Jobs :
     state[j] = "running" =>
        \A d \in Deps[j] :
