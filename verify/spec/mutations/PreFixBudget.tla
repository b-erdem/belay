...
VARIABLES
  ...
  freshAttempt \* [Jobs -> BOOLEAN]  no step body has run since claim

vars == << state, attempt, journaled, spent, cancelReq, live, execCount, clock, freshAttempt >>
...
Insert(j) ==
  ...
  /\ UNCHANGED << attempt, journaled, spent, cancelReq, live, execCount, clock, freshAttempt >>
...
ClaimJob(j) ==
  ...
  /\ freshAttempt' = [freshAttempt EXCEPT ![j] = TRUE]
...
Exec(j) ==
  ...
  /\ ~cancelReq[j]
  /\ NextIdx(j) # 0
  /\ (spent[j] <= Budget[j] \/ freshAttempt[j])
  ...
  /\ freshAttempt' = [freshAttempt EXCEPT ![j] = FALSE]
...
Crash(j) ==
  ...
  /\ UNCHANGED << state, attempt, journaled, spent, cancelReq, execCount, clock, freshAttempt >>

\* advance: cosmetic clock tick (leases/backoff are abstracted).
Advance ==
  /\ clock < MaxClock
  /\ clock' = clock + 1
  /\ UNCHANGED << state, attempt, journaled, spent, cancelReq, live, execCount, freshAttempt >>
...
CancelRunning(j) ==
  ...
  /\ UNCHANGED << state, attempt, journaled, spent, live, execCount, clock, freshAttempt >>
...
Raise(j) ==
  /\ state[j] = "running"
  /\ live[j]
  \* Faithful to pre-fix code: after the crossing step journals, control
  \* never returns to worker code (the post-write check throws), so a
  \* worker raise cannot intervene -- only a crash can abandon the attempt.
  /\ spent[j] <= Budget[j]
  /\ state' = [state EXCEPT ![j] =
                 IF attempt[j] >= MaxAttempts[j] THEN "failed" ELSE "ready"]
  /\ UNCHANGED << attempt, journaled, spent, cancelReq, live, execCount, clock, freshAttempt >>
