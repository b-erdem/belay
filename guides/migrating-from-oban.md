...
| Oban | Capstan |
|---|---|
| `use Oban.Worker, queue: :q, max_attempts: n` | `use Capstan.Worker, queue: :q, max_attempts: n` |
| `perform(%Oban.Job{args: args})` | `run(%Capstan.Ctx{job: %{input: input}})` |
...
the worker; Capstan stores unique *keys* on rows. Move the intent to the
insert site:

```elixir
# Oban: unique: [keys: [:run_id], states: :incomplete] on the worker
# Capstan: a key at insert
Capstan.insert(MyApp.Capstan, MyWorker.new(%{"run_id" => id}, unique: "ingest:#{id}"))
```
...
Run `Capstan.Storage.Postgres.migrate!(db_url)` at deploy (idempotent).
...
```bash
mix capstan.migrate_oban --url postgres://.../my_app          # dry run: a report
...
rows polluting Capstan's metrics; where they are, they audit fine:
...
Capstan treat unknown ids as dead, which is the correct answer for them.
...
- **Tests**: run Capstan queues as `manual: true` in the test env and
  drive them with `Capstan.Testing.drain` — it executes workers in the
  test process, so Ecto sandbox access just works. Truncate `capstan_*`
  tables between tests through Capstan's own pool (it is outside the
  sandbox by design).
