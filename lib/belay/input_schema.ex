...
defmodule Belay.InputSchema do
  @moduledoc """
  ...
  def validate!(worker, schema, input) when is_map(input) do
    ...
    case errors do
      [] -> input
      errors -> raise Belay.InputError, worker: worker, errors: Enum.reverse(errors)
    end
  end

  def validate!(worker, _schema, input) do
    raise Belay.InputError,
      worker: worker,
