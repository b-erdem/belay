defmodule Belay.Runner do
  ...
  #
  # Also implements the in-job APIs surfaced on `Belay`: step, await, sleep,
  # spawn/await_children, emit, debit, steering — including their replay-mode
  # behavior (see `Belay.Replay`).

  require Logger

  alias Belay.{Config, Ctx, Job, Worker}
  ...
  def execute(%Config{} = config, %Job{} = job) do
    ...
    ctx = %Ctx{job: job, belay: config.name, config: config}
    started = System.monotonic_time()

    :telemetry.execute([:belay, :job, :start], %{system_time: System.system_time()}, %{
      job: job,
      name: config.name
    })
    ...
    case storage.ack(ref, job, outcome, Config.now(config)) do
      ...
      {:error, :stale} ->
        Logger.warning("[belay] stale ack for job #{job.id} (attempt #{job.attempt})")
  ...
  def execute_chunk(%Config{} = config, [%Job{} | _] = jobs) do
    ...
    ctxs = Enum.map(jobs, &%Ctx{job: &1, belay: config.name, config: config})
  ...
  defp with_timeout(module, fun) do
    case module.__belay_defaults__()[:timeout] do
      nil ->
        fun.()
      ...
      timeout ->
        ...
        task =
          Task.async(fn ->
            try do
              ...
              :throw, {:belay_control, control} -> {:control, control}
        ...
        case Task.yield(task, timeout_ms(timeout)) || Task.shutdown(task, :brutal_kill) do
          ...
          {:ok, {:control, control}} -> throw({:belay_control, control})
  ...
  defp error_outcome({kind, reason, stacktrace}, config, job) do
    ...
    :telemetry.execute([:belay, :job, :exception], %{}, %{
      job: job,
      name: config.name,
      kind: kind,
      reason: reason,
  ...
  def step(%Ctx{job: job, config: config} = ctx, name, fun, opts) do
    ...
    case storage.get_step(ref, job.id, name) do
      {:ok, bin} ->
        Belay.Codec.decode(bin)

      :none when ctx.replay? ->
        throw({:belay_replay, {:missing_step, name}})
  ...
  def await(%Ctx{job: job, config: config} = ctx, name, opts) do
    ...
    case storage.get_signal(ref, scopes, name) do
      ...
      :none when ctx.replay? ->
        throw({:belay_replay, {:blocked_on_signal, name}})
      ...
      :none ->
        if job.await_name == name and deadline_passed?(job, config) do
          ...
          throw({:belay_control, {:await, hd(scopes), name, deadline}})
        end
    end
  end
  ...
  def sleep(%Ctx{config: config} = ctx, name, seconds)
    ...
    if DateTime.compare(now, target) == :lt and not ctx.replay? do
      throw({:belay_control, {:sleep, max(DateTime.diff(target, now, :second), 1)}})
    end
    ...
    step(
      ...
      fn ->
        keyed =
          buildables
          |> Enum.with_index()
          |> Enum.map(fn {{Belay.Worker, worker, input, opts}, index} ->
            ...
            {key, {Belay.Worker, worker, input, opts}}
          end)

        _inserted = Belay.insert_all(ctx.belay, Enum.map(keyed, &elem(&1, 1)))
  ...
  def await_children(%Ctx{job: job, config: config} = ctx) do
    ...
    cond do
      ...
      ctx.replay? ->
        throw({:belay_replay, {:blocked_on_children, length(children)}})
      ...
      true ->
        ...
        if children != [] and Enum.all?(children, &Job.terminal?/1) do
          ...
          throw({:belay_control, {:await, "job:#{job.id}", "$children", nil}})
        end
    end
  end
  ...
  def emit(%Ctx{job: job, config: config}, payload) when is_map(payload) do
    ...
    Registry.dispatch(Belay.run_registry(config.name), {:events, job.id}, fn entries ->
      for {pid, _} <- entries, do: send(pid, {:belay_event, job.id, seq, payload})
    end)
  ...
  def debit(%Ctx{job: job, config: config}, resource, units) when is_integer(units) do
    ...
    case spec.rate do
      %{resource: ^resource, period: period, estimate: estimate} ->
        ...
        credit =
          if Process.get({:belay_credited, job.id}) do
            0
          else
            Process.put({:belay_credited, job.id}, true)
            estimate
          end
  ...
  defp check_cancel!(config, job) do
    ...
    case storage.get_job(ref, job.id) do
      {:ok, %Job{cancel_requested: true}} ->
        throw({:belay_control, {:cancelled, :cancel_requested}})
  ...
  defp register_running(config, job_id) do
    Registry.register(Belay.run_registry(config.name), :running, job_id)
  rescue
  ...
  defp broadcast_terminal(config, jobs) do
    for %Job{} = job <- jobs, job.state in ~w(succeeded failed cancelled) do
      Registry.dispatch(Belay.run_registry(config.name), {:result, job.id}, fn entries ->
        for {pid, _} <- entries, do: send(pid, {:belay_result, job.id, job})
      end)

      Belay.Notifier.broadcast_all(config, {:result, job.id})
    end
  rescue
    ArgumentError -> :ok
  end
