defmodule Belay.MCP do
  ...
  database access — no running Belay instance required.
  """
  ...
  def handle_request(%{"method" => "initialize", "id" => id}, _storage, _opts) do
    result(id, %{
      ...
      "capabilities" => %{"tools" => %{}},
      "serverInfo" => %{"name" => "belay", "version" => version()}
    })
  end
  ...
  @doc false
  defdelegate job_summary(job), to: Belay.View
  @doc false
  defdelegate job_detail(job), to: Belay.View
  @doc false
  defdelegate step_summary(step), to: Belay.View
  @doc false
  defdelegate event_summary(event), to: Belay.View
  ...
  defp version, do: Application.spec(:belay, :vsn) |> to_string()
end

defmodule Mix.Tasks.Belay.Mcp do
  @shortdoc "Serve the Belay MCP server over stdio"

  @moduledoc """
  Serve the Belay MCP server over stdio against a Postgres database:

      mix belay.mcp --url postgres://user:pass@host:5432/my_app

  The URL may also come from the `BELAY_URL` environment variable. Add it to
  ...
  def run(argv) do
    ...
    url =
      opts[:url] || System.get_env("BELAY_URL") ||
        Mix.raise("pass --url postgres://... or set BELAY_URL")
    ...
    conn_opts =
      ...
      |> Keyword.merge(
        name: Belay.MCP.Conn,
        pool_size: 2,
        types: Belay.Storage.PostgresTypes
      )

    {:ok, _} = Postgrex.start_link(conn_opts)

    Belay.MCP.serve({Belay.Storage.Postgres, Belay.MCP.Conn},
