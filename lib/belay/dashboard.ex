defmodule Belay.Dashboard do
  ...
  @impl GenServer
  def init(opts) do
    belay = Keyword.fetch!(opts, :belay)
    ...
    Logger.info("[belay] dashboard for #{inspect(belay)} on http://localhost:#{actual_port}")
  ...
  defp respond(socket, state, method, route, query, headers, body) do
    ...
    error ->
      Logger.warning("[belay] dashboard request failed: #{Exception.message(error)}")
  ...
  defp handle(:GET, "/", _query, _body, _state) do
    {200, {:html, Belay.Dashboard.Page.html()}}
  end

  defp handle(:GET, "/api/overview", _query, _body, state) do
    jobs = Belay.list_jobs(state.belay, limit: 30)
    {storage, ref} = Belay.Config.fetch!(state.belay).storage_ref
  ...
  defp handle(:GET, "/api/jobs", query, _body, state) do
    ...
    {200, %{"jobs" => state.belay |> Belay.list_jobs(filters) |> Enum.map(&View.job_summary/1)}}
  end

  defp handle(:GET, "/api/jobs/" <> id, _query, _body, state) do
    id = String.to_integer(id)

    case Belay.get_job(state.belay, id) do
      {:ok, job} ->
        {:ok, steps} = Belay.steps(state.belay, id)
        events = Belay.events(state.belay, id)
        children = Belay.list_jobs(state.belay, parent_id: id)
  ...
  defp handle(:GET, "/api/workflows/" <> workflow_id, _query, _body, state) do
    jobs = Belay.Workflow.jobs(state.belay, workflow_id)

    {200,
     %{
       "workflow_id" => workflow_id,
       "jobs" => Enum.map(jobs, &View.job_summary/1),
       "done" => jobs != [] and Enum.all?(jobs, &Belay.Job.terminal?/1)
     }}
  end
  ...
  defp handle(:POST, "/api/signals", _query, body, state) do
    ...
    with :ok <- authorize_mutation(state, "signal", params) do
      :ok = Belay.signal(state.belay, scope, name, Map.get(params, "payload", %{}))
  ...
  defp mutate(state, "retry", id, params) do
    with :ok <- authorize_mutation(state, "retry_job", Map.put(params, "id", id)) do
      case Belay.retry_job(state.belay, id) do
  ...
  defp queue_specs(state) do
    config = Config.fetch!(state.belay)

    static = Map.new(config.queues, fn {queue, spec} -> {queue, spec_view(spec)} end)

    dynamic =
      try do
        Map.new(Belay.Queues.dynamic_specs(config), fn {queue, spec} ->
