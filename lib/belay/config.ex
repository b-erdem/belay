defmodule Belay.Config do
  @moduledoc """
  The resolved runtime configuration of a Belay instance.

  Built from the options passed to `Belay.start_link/1`; carried through
  ...
  defstruct name: Belay,
            storage: nil,
            storage_ref: nil,
            queues: %{},
            crons: [],
            clock: Belay.Clock.System,
            node_id: nil,
            poll_interval: 500,
            busy_poll: 25,
            notifiers: [{Belay.Notifier.Local, []}],
            lease_ttl: 30_000,
  ...
  def new(opts) do
    ...
    name = Keyword.get(opts, :name, Belay)
    ...
    %__MODULE__{
      ...
      clock: Keyword.get(opts, :clock, Belay.Clock.System),
  ...
  # `otp_app:` reads `config :my_app, MyApp.Belay, ...` from application
  ...
  def now(%__MODULE__{clock: clock}), do: Belay.Clock.now(clock)

  def put(%__MODULE__{name: name} = config) do
    :persistent_term.put({Belay, name}, config)
    config
  end

  def fetch!(name) do
    :persistent_term.get({Belay, name})
  rescue
    ArgumentError ->
      reraise(ArgumentError, "no Belay instance named #{inspect(name)}", __STACKTRACE__)
  end
  ...
  defp storage_module(:memory), do: Belay.Storage.Memory
  defp storage_module(:postgres), do: Belay.Storage.Postgres
  ...
  defp normalize_notifiers(notifiers, opts) do
    ...
    Enum.map(notifiers, fn entry ->
      {module, nopts} =
        case entry do
          :local ->
            {Belay.Notifier.Local, []}

          :postgres ->
            {Belay.Notifier.Postgres, []}
      ...
      if module == Belay.Notifier.Postgres and storage_mod != Belay.Storage.Postgres do
  ...
  defp queue_defaults(queue, limit, opts) do
    ...
    %{
      ...
      # Manual queues get no producer — claimed only via Belay.Testing.drain.
  ...
  defp normalize_rate(opts) do
    %{
      ...
      # resource units, corrected post-hoc via Belay.debit/3.
  ...
  defp normalize_crons(crons) do
    Enum.map(crons, fn cron ->
      %{
        ...
        expr: Belay.CronExpr.parse!(Keyword.fetch!(cron, :expr)),
