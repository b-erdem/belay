defmodule Belay.Testing do
  ...
  def drain(name, queue, opts \\ []) do
    ...
    spec = Belay.Queues.resolve_spec!(config, queue)
  ...
  defp chunked?(job) do
    Belay.Job.worker_module!(job).__belay_defaults__()[:chunk] != nil
  rescue
    _ -> false
  end

  defp chunk_size(job) do
    Belay.Job.worker_module!(job).__belay_defaults__()[:chunk]
    |> Keyword.get(:size, 10)
  end
end
