...
defmodule Belay.Crons do
  ...
  def put(name, cron_name, expression, worker, opts \\ []) do
    ...
    entry = %{
      ...
      opts: Belay.Queues.encode_opts(Keyword.drop(opts, [:input]))
    }
        ...
        with {:ok, expr} <- CronExpr.parse(entry.expression),
          ...
          [
            %{
              ...
              worker: worker,
              input: entry.input || %{},
              opts: Belay.Queues.decode_opts(entry.opts || %{})
            }
          ]
        else
          _ ->
            ...
            Logger.warning("[belay] skipping invalid dynamic cron #{inspect(entry.name)}")
...
defmodule Belay.QueueSync do
  ...
  defp sync(%{config: config} = state) do
    dynamic =
      try do
        Belay.Queues.dynamic_specs(config)
      rescue
        error ->
          Logger.warning("[belay] queue sync skipped: #{Exception.message(error)}")
  ...
  defp apply_diff(%{config: config, running: running} = state, desired) do
    sup = Belay.producer_sup(config.name)
    ...
    running =
      Enum.reduce(desired, running, fn {queue, spec}, running ->
        case running do
          ...
          _ ->
            case DynamicSupervisor.start_child(sup, {Belay.Producer, {config, queue, spec}}) do
              {:ok, pid} ->
              ...
              {:error, reason} ->
                Logger.warning("[belay] producer #{queue} failed to start: #{inspect(reason)}")
