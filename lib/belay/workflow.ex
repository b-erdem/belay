defmodule Belay.Workflow do
  ...
  def add(%__MODULE__{} = workflow, name, {Belay.Worker, worker, input, opts}, wf_opts \\ []) do
    name = to_string(name)
  ...
  def insert(%__MODULE__{} = workflow, name) do
    ...
    rows =
      ...
      |> Enum.map(fn entry ->
        ...
        Belay.Job.new(entry.worker, entry.input, opts, entry.worker.__belay_defaults__())
      end)

    {:ok, jobs} = storage.insert_jobs(ref, rows, now)

    jobs |> Enum.map(& &1.queue) |> Enum.uniq() |> Enum.each(&Belay.poke(config, &1))
