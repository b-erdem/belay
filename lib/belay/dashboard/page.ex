defmodule Belay.Dashboard.Page do
  @moduledoc false
  ...
  <body>
  <header>
    <h1>⚓ Belay</h1><span class="inst mono" id="inst"></span><span class="live" id="live">● live</span>
  </header>
  <main>
  ...
  if (urlToken) {
    try { sessionStorage.setItem('belay:token', urlToken); } catch (e) {}
    qs.delete('token');
    history.replaceState(null, '', location.pathname + (qs.size ? '?' + qs : '') + location.hash);
  }
  let stored = null;
  try { stored = sessionStorage.getItem('belay:token'); } catch (e) {}
  ...
  const histKey = () => 'belay:' + (overview ? overview.instance : '');
