defmodule Capstan.Replay do
  @moduledoc """
  Time-travel debugging: re-run a job's *code* against its *recorded journal*.

      case Capstan.Replay.dry_run(MyCapstan, job_id) do
  ...
  def dry_run(name, job_id) do
    ...
    with {:ok, job} <- fetch(storage, ref, job_id) do
      ...
      try do
        ...
        :throw, {:capstan_replay, reason} -> {:blocked, reason, trace}
        :throw, {:capstan_control, control} -> {:blocked, {:control, elem(control, 0)}, trace}
