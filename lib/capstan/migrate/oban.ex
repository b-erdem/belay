defmodule Capstan.Migrate.Oban do
  @moduledoc """
  Move an Oban installation's **pending work** into Capstan.

  Converts `available`, `scheduled`, and `retryable` rows from `oban_jobs`
  into `ready` Capstan jobs (scheduled/retryable keep their `scheduled_at`
  as `ready_at`). Everything else is deliberately left alone:
    ...
    * Terminal rows (`completed`, `discarded`, `cancelled`) are history, not
      ...
      pollute Capstan's metrics while buying nothing. Keep the old table
  ...
  Capstan stores unique *keys* on rows. The analyzer lists workers whose
  pending rows may need `unique:` re-declared at their insert sites.

  Used by `mix capstan.migrate_oban`; callable directly:

      {:ok, conn} = Postgrex.start_link(...)
      report = Capstan.Migrate.Oban.analyze(conn)
      Capstan.Migrate.Oban.execute(conn, report)
  """
  ...
  Capstan worker module are migrated unless `force: true`.
  """
  ...
  # Oban errors are a jsonb[] of %{"at", "attempt", "error"}; Capstan errors
  ...
  defp ported?(kind) do
    ...
    Code.ensure_loaded?(module) and function_exported?(module, :__capstan_defaults__, 0)
  end
