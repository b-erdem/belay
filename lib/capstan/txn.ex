defmodule Capstan.Txn do
  ...
  a job exists if and only if the business write committed.
      ...
      Postgrex.transaction(MyApp.Pool, fn conn ->
        ...
        {:ok, _job} =
          Capstan.Txn.insert(conn, MyApp.Capstan, MyApp.FulfillOrder.new(%{"id" => order.id}))
      end)
  ...
  Works identically with an Ecto repo (anything exporting `query!/2`):
      ...
      MyApp.Repo.transaction(fn ->
        ...
        {:ok, _job} =
          Capstan.Txn.insert(MyApp.Repo, MyApp.Capstan, MyApp.FulfillOrder.new(%{"id" => order.id}))
      end)

  Capstan itself takes no Ecto dependency — the bridge is duck-typed over
  ...
  **Wake-ups are transactional too.** When the instance has the `:postgres`
  ...
  Requires the named Capstan instance to be running in this VM (it supplies
  worker defaults, the clock, and notifier configuration), and Postgres
  storage.
  """

  alias Capstan.{Config, Job}
  alias Capstan.Notifier
  alias Capstan.Storage.Postgres

  @doc "Insert one job in the caller's transaction. Returns `{:ok, job}` (duplicates flagged)."
  def insert(conn_or_repo, name, {Capstan.Worker, _worker, _input, _opts} = buildable) do
  ...
  def insert_all(conn_or_repo, name, buildables) when is_list(buildables) do
    ...
    unless elem(config.storage, 0) == Postgres do
      raise ArgumentError, "Capstan.Txn requires the :postgres storage adapter"
    end
