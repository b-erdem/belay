defmodule Belay do
  ...
  alias Belay.{Config, Ctx, Job, Runner}
  ...
  def insert(name, {Belay.Worker, worker, input, opts}) do
  ...
  def pause_queue(name, queue), do: call_producer(name, queue, :belay_pause)

  @doc "Resume a paused queue."
  @spec resume_queue(instance(), atom() | String.t()) :: :ok | {:error, :no_producer}
  def resume_queue(name, queue), do: call_producer(name, queue, :belay_resume)
    ...
    case check.() do
      :pending ->
        ...
        if remaining <= 0 do
          ...
          receive do
            {:belay_result, ^id, _info} -> await_loop(check, config, id, deadline)
          after
  ...
  def map_children(%Ctx{} = ctx, name, worker, inputs, opts \\ []) do
    ids = Runner.spawn_many(ctx, name, Enum.map(inputs, &{Belay.Worker, worker, &1, opts}))
  ...
  def poke(%Config{} = config, queue) do
    Belay.Notifier.broadcast_all(config, {:poke, to_string(queue)})
  end
