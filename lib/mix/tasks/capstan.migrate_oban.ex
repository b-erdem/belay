defmodule Mix.Tasks.Capstan.MigrateOban do
  @shortdoc "Migrate Oban's pending jobs into Capstan (dry-run by default)"

  @moduledoc """
  Move pending Oban work into Capstan, safely.

      # See what would happen (no writes):
      mix capstan.migrate_oban --url postgres://localhost/my_app

      # Do it (stop Oban's producers first):
      mix capstan.migrate_oban --url postgres://localhost/my_app --execute

      # Renamed a worker during the port?
      mix capstan.migrate_oban --url ... --execute \\
  ...
  Run this task from your application (`mix capstan.migrate_oban` inside
  the app) so worker modules are loadable — the analyzer verifies each
  pending worker is actually ported to `Capstan.Worker` and refuses to
  migrate ones that aren't (override with `--force`).

  Uniqueness is the one thing that cannot migrate mechanically: Oban keeps
  unique *policy* on workers, Capstan keeps unique *keys* on rows. The
  ...
  def run(argv) do
    ...
    cond do
      ...
      opts[:execute] ->
        result = Capstan.Migrate.Oban.execute(conn, report, force: opts[:force] || false)
        ...
        Next steps:
          1. Keep Oban stopped; deploy with Capstan producers on.
