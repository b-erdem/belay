defmodule Belay.RetrySemanticsTest.CancelSelfOnce do
  @moduledoc false
...
defmodule Belay.RetrySemanticsTest do
  ...
  alias Belay.RetrySemanticsTest.CancelSelfOnce
  alias Belay.Test.{FailN, Tagged}
  ...
  test "retry supersedes a stale pending cancel", %{name: name, clock: clock} do
    {:ok, job} = Belay.insert(name, Tagged.new(%{"tag" => "s1"}, max_attempts: 1))
    ...
    {:ok, retried} = Belay.retry_job(name, job.id)
  ...
  test "retrying a cascade-cancelled dependent re-holds it; deps still gate it",
    ...
    {:ok, _} = Belay.retry_job(name, jobs["b"].id)
