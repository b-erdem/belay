defmodule Belay.ChunkTest do
  use Belay.Test.Case, async: false

  alias Belay.Test.{ChunkBoom, ChunkEcho, ChunkFlaky, ChunkGather, ChunkNoImpl}
  ...
  test "jobs run in size-bounded chunks with per-job results", %{name: name} do
    jobs =
      for n <- 1..7 do
        {:ok, job} = Belay.insert(name, ChunkEcho.new(%{"n" => n}))
        job
      end
    ...
    assert {:ok, 8} = Belay.await_result(name, fourth.id, 100)
  end

  test "partial failure retries only the failed jobs", %{name: name, clock: clock} do
    for n <- 1..3, do: {:ok, _} = Belay.insert(name, ChunkFlaky.new(%{"n" => n}))
    {:ok, flaky} = Belay.insert(name, ChunkFlaky.new(%{"n" => 99, "fail" => true}))
    ...
    assert Events.count({:chunk, 1}) == 1
    assert {:ok, 99} = Belay.await_result(name, flaky.id, 100)
  end

  test "a whole-chunk error retries every job on its own backoff", %{name: name, clock: clock} do
    for _ <- 1..4, do: {:ok, _} = Belay.insert(name, ChunkBoom.new(%{}))
  ...
  test "chunk: without run_chunk/1 fails jobs with a config error", %{name: name} do
    {:ok, job} = Belay.insert(name, ChunkNoImpl.new(%{}))
  ...
  test "live producer gathers up to the window, dispatches full chunks at once" do
    %{name: name} = start_belay!(sim_clock: false, queues: [default: 10], poll_interval: 40)

    # 2 jobs < size 3: gathered, dispatched together at the ~800ms deadline.
    for n <- 1..2, do: {:ok, _} = Belay.insert(name, ChunkGather.new(%{"n" => n}))

    wait_until(fn -> Events.count({:gathered, 2}) == 1 end, 3_000)

    # 3 more: a full chunk dispatches without waiting for the window.
    started = System.monotonic_time(:millisecond)
    for n <- 3..5, do: {:ok, _} = Belay.insert(name, ChunkGather.new(%{"n" => n}))
