defmodule Belay.DynamicTest do
  use Belay.Test.Case, async: false

  alias Belay.{Crons, Queues}
  alias Belay.Test.{CronJob, Echo}

  test "dynamic queues start producers at runtime and stop on delete" do
    %{name: name} =
      start_belay!(sim_clock: false, queues: [], dynamic_sync: 100, poll_interval: 100)

    :ok = Queues.put(name, :imports, limit: 5)

    wait_until(fn ->
      Registry.lookup(Belay.registry(name), {:producer, "imports"}) != []
    end)

    {:ok, job} = Belay.insert(name, Echo.new(%{"dyn" => true}, queue: :imports))

    assert {:ok, %{"dyn" => true}} = Belay.await_result(name, job.id, 3_000)
  ...
  test "drain resolves dynamic queue specs" do
    ...
    {:ok, _} = Belay.insert(name, Echo.new(%{}, queue: :dyn_manual))
  ...
  test "dynamic crons fire (deduped) and pause persists" do
    %{name: name} =
      start_belay!(sim_clock: false, queues: [default: 5], cron_interval: 50)
  ...
  test "a dynamic cron overrides a static one by name" do
    %{name: name} = start_belay!(crons: [[name: "x", expr: "0 0 1 1 *", worker: CronJob]])

    :ok = Crons.put(name, "x", "30 6 * * *", CronJob)

    entries = Belay.Crons.schedule_entries(Belay.Config.fetch!(name))
