defmodule Belay.AdaptiveLimitTest do
  use Belay.Test.Case, async: false

  alias Belay.Test.Sleeper
  ...
  test "producer scales up under sustained load and decays when idle" do
    ...
    for _ <- 1..12 do
      {:ok, _} = Belay.insert(name, Sleeper.new(%{"ms" => 40}))
    end
    ...
    wait_until(fn -> Belay.stats(name)["default"]["succeeded"] == 12 end, 5_000)
