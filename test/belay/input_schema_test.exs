defmodule Belay.InputSchemaTest do
  use ExUnit.Case, async: true

  alias Belay.InputSchema
  ...
  test "integers satisfy :float but not vice versa" do
    ...
    assert_raise Belay.InputError, ~r/count expected :integer/, fn ->
  ...
  test "collects every violation into one precise error" do
    error =
      assert_raise Belay.InputError, fn ->
  ...
  test "non-map input fails with a clear message" do
    assert_raise Belay.InputError, ~r/must be a map/, fn ->
