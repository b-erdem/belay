...
if Application.get_env(:belay, :test_storage) == :postgres do
  defmodule Belay.TxnTest do
    ...
    test "a rollback aborts the enqueue; a commit lands it", %{} do
      ...
      assert {:ok, %{"txn" => "committed"}} = Belay.await_result(name, job.id, 100)
    end
    ...
    test "an Ecto-shaped repo module works through the same bridge" do
      ...
      assert %{succeeded: 1} = Testing.drain(name, :default)
      assert {:ok, %{"via" => "repo"}} = Belay.await_result(name, job.id, 100)
    end
    ...
    test "with the :postgres notifier, the wake-up delivers on commit only" do
      ...
      inserter = start_belay!(sim_clock: false, queues: [], notifiers: [:local, :postgres])
      ...
      assert {:ok, %{"woken" => "by-commit"}} =
               Belay.await_result(inserter.name, job.id, 5_000)
