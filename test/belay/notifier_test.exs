...
if Application.get_env(:belay, :test_storage) == :postgres do
  defmodule Belay.NotifierTest do
    use Belay.Test.Case, async: false

    alias Belay.Test.Echo

    # Two Belay instances in one BEAM share the database but nothing else —
    # no registry, no :pg scope. Only pg_notify can carry the wake-up.
    defp pair!(notifiers) do
      inserter =
        start_belay!(sim_clock: false, queues: [], notifiers: notifiers)
    ...
    test "pg_notify carries pokes and results across unconnected instances" do
      ...
      {:ok, job} = Belay.insert(inserter, Echo.new(%{"x" => 1}))

      assert {:ok, %{"x" => 1}} = Belay.await_result(inserter, job.id, 5_000)
    ...
    test "without the postgres notifier, unconnected instances are poll-bound" do
      ...
      {:ok, job} = Belay.insert(inserter, Echo.new(%{}))

      assert {:error, :timeout} = Belay.await_result(inserter, job.id, 1_000)
