defmodule Belay.DashboardTest do
  use Belay.Test.Case, async: false

  alias Belay.Test.{Emitter, Tagged}
  ...
  setup do
    ...
    port = Belay.Dashboard.port(dash)
  ...
  test "overview, job detail, and workflow endpoints return real data", %{
    ...
    {:ok, emitted} = Belay.insert(name, Emitter.new(%{}))
  ...
  test "mutations work and respect the authorizer", %{base: base, name: name} do
    {:ok, failed} =
      Belay.insert(name, Tagged.new(%{"tag" => "x", "fail" => true}, max_attempts: 1))
  ...
  test "SSE stream authenticates and pushes an overview frame immediately",
    ...
    {:ok, _} = Belay.insert(name, Tagged.new(%{"tag" => "sse"}))
    Testing.drain(name, :default)
  ...
  test "tokenless dashboards are read-only by default", %{name: name} do
    {:ok, read_only} = Belay.Dashboard.start_link(belay: name, port: 0)
    read_only_base = "http://localhost:#{Belay.Dashboard.port(read_only)}"
