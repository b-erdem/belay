defmodule Belay.ResourceRateTest do
  # Token-style resource budgets: admission on an estimate, corrected by
  # actual usage (Belay.debit/3) so the window converges on reality.
  use Belay.Test.Case, async: false

  alias Belay.Test.ResourceUser
  ...
  defp claim!(name, demand) do
    ...
    conf = config(name)
    spec = Belay.Config.queue_spec(conf, :metered)
  ...
  defp window_total(name) do
    ...
    case mod do
      Belay.Storage.Memory ->
        :sys.get_state(ref).rate |> Map.values() |> Enum.sum()

      Belay.Storage.Postgres ->
        %{rows: [[sum]]} =
          Postgrex.query!(ref, "SELECT COALESCE(sum(count), 0) FROM belay_rate", [])
  ...
  test "admission divides the window by the estimate", %{name: name} do
    for i <- 1..5 do
      {:ok, _} = Belay.insert(name, ResourceUser.new(%{"actual" => 1_000 + i}, queue: :metered))
    end
  ...
  test "actual usage trues the window up, opening real headroom", %{name: name} do
    jobs =
      for i <- 1..5 do
        {:ok, job} = Belay.insert(name, ResourceUser.new(%{"actual" => 1_000}, queue: :metered))
        _ = i
        job
      end
    ...
    for job <- claimed do
      {:ok, _, _} = Belay.Runner.execute(config(name), job)
    end
