defmodule Belay.LeasesTest do
  use Belay.Test.Case, async: false

  alias Belay.Test.{Echo, Tagged}
  alias Belay.Workflow

  setup do
    {:ok, start_belay!(lease_ttl: 10_000)}
  end
  ...
  defp claim_one!(name) do
    ...
    {:ok, [job]} = mod.claim(ref, spec, 1, "n1", 10_000, Belay.Config.now(conf))
  ...
  test "expired leases are reclaimed for retry with attempts intact", %{
    ...
    {:ok, inserted} = Belay.insert(name, Echo.new(%{"n" => 1}))
  ...
  test "renewed leases do not expire", %{name: name, clock: clock} do
    {:ok, inserted} = Belay.insert(name, Echo.new(%{}))
    ...
    {mod, ref} = storage(name)
    conf = config(name)
    until = DateTime.add(Belay.Config.now(conf), 10, :second)
  ...
  test "stale acks from zombie executors are fenced off", %{name: name, clock: clock} do
    {:ok, _} = Belay.insert(name, Echo.new(%{}))
