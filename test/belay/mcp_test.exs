defmodule Belay.MCPTest do
  use Belay.Test.Case, async: false

  alias Belay.MCP
  alias Belay.Test.{Awaiter, Emitter, Tagged}
  ...
  test "initialize and tools/list speak MCP", %{name: name} do
    assert %{"result" => %{"protocolVersion" => _, "serverInfo" => %{"name" => "belay"}}} =
             MCP.handle_request(
  ...
  test "an authorizer gates mutating tools but never introspection", %{name: name} do
    {:ok, job} =
      Belay.insert(name, Tagged.new(%{"tag" => "f", "fail" => true}, max_attempts: 1))
  ...
  test "mutation tools are disabled by default", %{name: name} do
    {:ok, job} = Belay.insert(name, Tagged.new(%{"tag" => "x"}))
  ...
  test "mutation tools operate jobs", %{name: name} do
    ...
    {:ok, waiting} = Belay.insert(name, Awaiter.new(%{}))
