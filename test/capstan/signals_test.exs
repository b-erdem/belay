defmodule Capstan.SignalsTest do
  use Capstan.Test.Case, async: false

  alias Capstan.Test.Awaiter
  ...
  test "await parks the job; a signal wakes it with the payload", %{name: name} do
    ...
    assert {:ok, %{"approved" => true}} = Capstan.await_result(name, job.id, 100)
  end

  test "a pre-delivered signal is found immediately", %{name: name} do
    {:ok, job} = Capstan.insert(name, Awaiter.new(%{}))

    Capstan.signal_job(name, job.id, :approval, %{"pre" => 1})

    assert %{succeeded: 1} = Testing.drain(name, :default)
    assert {:ok, %{"pre" => 1}} = Capstan.await_result(name, job.id, 100)
  end

  test "await deadlines resume with a timeout", %{name: name, clock: clock} do
    {:ok, job} = Capstan.insert(name, Awaiter.new(%{"timeout" => 60}))
    ...
    assert %{succeeded: 1} = Testing.drain(name, :default)
    assert {:ok, %{"timeout" => true}} = Capstan.await_result(name, job.id, 100)
  end

  test "cleared signals block again", %{name: name} do
    {:ok, job} = Capstan.insert(name, Awaiter.new(%{}))

    Capstan.signal_job(name, job.id, :approval, %{})
    Capstan.clear_signal(name, "job:#{job.id}", :approval)
  ...
  test "a signal landing while the job runs wakes it at park time", %{name: name} do
    ...
    assert %{succeeded: 1} = Testing.drain(name, :default)
    assert {:ok, %{"mid" => 1}} = Capstan.await_result(name, job.id, 100)
  end
end
