...
if Application.get_env(:capstan, :test_storage) == :postgres do
  defmodule Capstan.AdapterEquivalenceTest do
    use ExUnit.Case, async: false

    alias Capstan.{Clock, Job}
    ...
    defp materialize(:ack, state) do
      outcome =
        case :rand.uniform(6) do
          1 -> {:succeeded, Capstan.Codec.encode(%{ok: true})}
          2 -> :retry
          3 -> {:failed, %{"error" => "boom"}}
    ...
    defp start_side(:memory, seed, spec) do
      ...
      ref = Capstan.Storage.Memory.ref(name)
      {:ok, pid} = Capstan.Storage.Memory.start_link(ref)

      %{mod: Capstan.Storage.Memory, ref: ref, clock: clock, spec: spec, pid: pid, kind: :memory}
    end
    ...
    defp start_side(:postgres, _seed, spec) do
      ...
      opts =
        ...
        |> Keyword.merge(pool_size: 3, types: Capstan.Storage.PostgresTypes)
