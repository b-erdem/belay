defmodule ReproTimeoutRaiseWorker do
  @moduledoc false
  use Capstan.Worker, queue: :default, max_attempts: 3, timeout: {5, :second}
...
defmodule Capstan.VerifyFindingReproTest do
  ...
  defp drain_in_probe(name) do
    {pid, ref} =
      spawn_monitor(fn ->
        result = Capstan.Testing.drain(name, :default)
        exit({:drained, result})
      end)
  ...
  test "CONTROL: raising worker WITHOUT timeout drains to retry", %{name: name} do
    {:ok, job} = Capstan.insert(name, ReproPlainRaiseWorker.new(%{}))
  ...
  test "FINDING: raising worker WITH timeout kills executor, job stuck running", %{name: name} do
    {:ok, job} = Capstan.insert(name, ReproTimeoutRaiseWorker.new(%{}))
