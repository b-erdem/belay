...
if Application.get_env(:capstan, :test_storage) == :postgres do
  defmodule Capstan.MigrateObanTest do
    use Capstan.Test.Case, async: false

    alias Capstan.Migrate.Oban, as: Migrate

    defmodule PortedWorker do
      use Capstan.Worker, queue: :migrated
      @impl Capstan.Worker
      def run(_ctx), do: :ok
    end
    ...
    setup do
      ...
      ported = "Capstan.MigrateObanTest.PortedWorker"
    ...
    test "execute migrates only ported pending rows, converts faithfully, re-runs skip",
      ...
      jobs = Capstan.list_jobs(name, %{state: "ready", limit: 50})
    ...
    test "mapping renames kinds and counts as ported when the target is", %{conn: conn} do
      report =
        Migrate.analyze(conn,
          mapping: %{"Legacy.UnportedWorker" => "Capstan.MigrateObanTest.PortedWorker"}
        )
