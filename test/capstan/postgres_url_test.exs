defmodule Capstan.PostgresURLTest do
  ...
  test "decodes credentials and database names and maps connection query options" do
    opts =
      Postgres.parse_url(
        "postgresql://alice%40acme:p%3Aa%2Fss@[::1]:5544/my%20db" <>
          "?sslmode=require&connect_timeout=8000&application_name=capstan"
      )
    ...
    assert opts[:parameters][:application_name] == "capstan"
  end
  ...
  test "explicit storage options override URL-derived options" do
    ...
    %{start: {Postgrex, :start_link, [opts]}} =
      Postgres.child_spec(
        {%{name: Capstan.PostgresURLTest.Instance},
    ...
    assert opts[:name] == Postgres.ref(Capstan.PostgresURLTest.Instance)
  end
