defmodule Capstan.EventsReplayTest do
  use Capstan.Test.Case, async: false

  alias Capstan.Replay
  alias Capstan.Test.{Awaiter, Divergent, Emitter, StepFlaky}

  setup do
    :persistent_term.put({Capstan.Test.Divergent, :path}, :original)
  ...
  test "emitted events are delivered live and replayable", %{name: name} do
    ...
    Capstan.subscribe_events(name, job.id)

    Testing.drain(name, :default)

    assert_received {:capstan_event, _, 1, %{"chunk" => "token-1"}}
    assert_received {:capstan_event, _, 2, %{"chunk" => "token-2"}}
    assert_received {:capstan_event, _, 3, %{"chunk" => "token-3"}}

    all = Capstan.events(name, job.id)
    ...
    assert [%{seq: 3}] = Capstan.events(name, job.id, 2)
  end
  ...
  test "dry_run detects divergence after a code-path change", %{name: name} do
    {:ok, job} = Capstan.insert(name, Divergent.new(%{}))
